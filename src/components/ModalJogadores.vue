...
// ─── Estado ──────────────────────────────────────────────
const busca = ref("")
const fotos = ref({}) // cache de fotos buscadas por nome

// ─── Mapeamento posição do campo → categoria da API ──────
const mapaPosicoes = {
  GK:  "Goalkeepers",
  LB:  "Defenders", CB1: "Defenders", CB2: "Defenders", RB: "Defenders",
  CM1: "Midfielders", CM2: "Midfielders", CAM: "Midfielders",
  LW:  "Forwards", RW: "Forwards", ST: "Forwards",
}

// badge exibido no card de cada jogador
const badgePorCategoria = {
  Goalkeepers: "GK", Defenders: "DEF", Midfielders: "MID", Forwards: "ATK"
}

// ─── Busca a foto individual de um jogador via API ───────
async function carregarFoto(nome) {
  if (fotos.value[nome]) return
  try {
    const data = await getPlayerPhoto(nome)
    if (data?.image) fotos.value[nome] = data.image
  } catch { /* silencia erros de foto */ }
}
...
// ─── Lista de jogadores filtrada e ordenada ───────────────
const lista = computed(() => {
  ...
  // com busca → procura em todos os jogadores
  if (termo) {
    return [
      ...(props.jogadores.Goalkeepers ?? []).map(j => ({ ...j, posicao: "GK" })),
      ...(props.jogadores.Defenders   ?? []).map(j => ({ ...j, posicao: "DEF" })),
      ...(props.jogadores.Midfielders ?? []).map(j => ({ ...j, posicao: "MID" })),
      ...(props.jogadores.Forwards    ?? []).map(j => ({ ...j, posicao: "ATK" })),
    ]
      .filter(j => normalizar(j.name).includes(termo))
      .slice(0, 50)
  }

  // sem busca → mostra apenas jogadores da posição selecionada
  const base = categoria ? (props.jogadores[categoria] ?? []) : []
  const badge = badgePorCategoria[categoria] ?? ""
  return base.map(j => ({ ...j, posicao: badge })).slice(0, 50)
})

// ─── Pré-carrega fotos dos primeiros 20 itens da lista ───
watch(lista, (novaLista) => {
  novaLista.slice(0, 20).forEach(j => carregarFoto(j.name))
})

// ─── Fallback: esconde img e mostra ícone SVG ────────────
...
<template>
  <div class="modal-overlay fixed inset-0 bg-black/75 backdrop-blur-sm flex justify-center items-center z-50" @click.self="emit('fechar')">
    <div class="modal-box bg-zinc-900 w-[420px] max-w-[95vw] rounded-2xl shadow-2xl overflow-hidden">
      ...
      <!-- Cabeçalho: título da posição e botão fechar -->
      <div class="flex justify-between items-center px-5 py-4 border-b border-zinc-800">
        <div>
          ...
          <p class="text-xs text-zinc-400 mt-0.5">{{ lista.length }} jogadores encontrados</p>
        </div>
        <button @click="emit('fechar')" class="w-8 h-8 flex items-center justify-center rounded-full bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-white transition-colors">
      ...
      <div class="max-h-80 overflow-y-auto px-4 pb-4 space-y-1.5">
        ...
        >
          <!-- Foto do jogador com fallback para ícone SVG -->
          ...
          <div class="flex-1 min-w-0">
            <p class="font-semibold truncate group-hover:text-black">{{ j.name }}</p>
            <p class="text-xs text-zinc-400 group-hover:text-black/60 truncate">{{ j.team }}</p>
          </div>
